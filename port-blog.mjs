...
const ARTICLES = [
  ...
  { slug: 'desenvolvimento-socioemocional-bilingue', tag: '04 · Além do idioma' },
];
...
function extract(html, slug) {
  ...
  // cover url (1ª img.cover-img) — attrs em qualquer ordem
  const coverImgTag = (html.match(/<img\b[^>]*class="cover-img"[^>]*>/) || [, ''])[0];
  meta.coverUrl = (coverImgTag.match(/src="([^"]+)"/) || [, ''])[1];

  // body: from H1 até o fim do <article>
  // Estrutura: <article> ... <h1>TITULO</h1> meta div ... <img cover> <article> CORPO </article></article>
  // O corpo "real" está no SEGUNDO <article>. Extraímos do primeiro </article>... <h1>
  const bodyMatch = html.match(/<article>\s*([\s\S]*?)\s*<\/article>\s*<!--\s*Lead Capture/);
  if (!bodyMatch) throw new Error(`não achei corpo de ${slug}`);
  meta.bodyHTML = bodyMatch[1];

  // Limpa do corpo: o primeiro H1 e o meta-info bar (já vai pro template)
  meta.bodyHTML = meta.bodyHTML
    .replace(/<h1>[^<]*<\/h1>\s*<div class="article-meta">[\s\S]*?<\/div>\s*<img[^>]*class="cover-img"[^>]*>\s*<article>\s*/, '')
    .trim();
...
function template({ slug, meta, tag, coverLocalPath }) {
  const url = `${CANONICAL_BG}/diario/${slug}/`;
  const canonicalCaxias = `${CANONICAL_CAXIAS}/blog/${slug}`;
...
<link rel="canonical" href="${canonicalCaxias}">
...
<style>
  ...
  *,*::before,*::after { box-sizing: border-box; }
  ...
  .brand-mark { width: 38px; height: 38px; filter: drop-shadow(0 4px 8px rgba(200,16,46,0.18)); }
  .brand-text { display: flex; flex-direction: column; line-height: 1.05; font-family: var(--font-display); }
  .brand-line-1 { font-weight: 600; font-size: .78rem; letter-spacing: .08em; text-transform: uppercase; color: var(--maple); }
  .brand-line-2 { font-weight: 500; font-size: 1.15rem; font-style: italic; color: var(--ink); }
  ...
  /* Lead form — hidden (no backend on BG yet) */
...
<header class="site-nav">
  ...
  <nav class="nav-links">
    ...
    <a href="/#abertura">Abertura 2027</a>
...
<article class="main">
  ...
  <div class="cover-frame">
    <img src="cover.jpg" alt="${meta.title}" loading="lazy">
  </div>
  ...
  <div class="cta-box">
    ...
    <p>Abrimos em 2027. Estamos recebendo famílias para conhecer o projeto pedagógico e reservar pré-matrícula.</p>
    ...
    <p class="urgency">Vagas limitadas para a turma inaugural 2027</p>
  </div>
...
<footer class="site-footer">
  <div class="footer-inner">
    <p><strong>Maple Bear Bento Gonçalves</strong> — Escola Bilíngue Canadense · Abertura prevista 2027</p>
...
async function main() {
  ...
  for (const art of ARTICLES) {
    ...
    // Render HTML
    const html = template({ slug: art.slug, meta, tag: art.tag });
    fs.writeFileSync(path.join(outDir, 'index.html'), html, 'utf8');
    console.log('✓ artigo gerado:', art.slug);
  }
